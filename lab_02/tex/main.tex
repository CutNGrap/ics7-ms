...
\img{0.6}{1}{Графики прямой $y = \hat{\mu}(\vec{x}_N)$, а также функций $y = \hat{\mu}(\vec{x}_n)$
	$y=\underline{{\mu}}(\vec{x}_n)$, $y=\overline{{\mu}}(\vec{x}_n)$,  как функций объема n выборки, где n изменяется от 10 до N}
\img{0.65}{2}{Графики прямой $z = S^2(\vec{x}_N)$, также
	функций $z = S^2(\vec{x}_n)$, $z = \underline{\sigma^2}(\vec{x}_n)$
	и $z = \overline{\sigma^2}(\vec{x}_n)$ как функций объема n выборки, где n
	изменяется от 10 до N}
\end{document}